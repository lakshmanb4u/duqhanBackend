...
public class CacheController {
    ...
    public void sheduledCache() {
        try {
            cacheProductList = new ObjectMapper().writeValueAsString(productService.getAllCacheProducts(
                        0, 300, new ProductRequistBean()));
    ...
    public void getCacheProductList(HttpServletResponse response,
    ...
    public static void buildProductBeanListPrice(Users u, String lp, String hp)
        ...
        for (ProductNewBean pnb : listOfProduct) {
            if ((pnb.getDiscountedPrice() < highPrice) &&
                    (pnb.getDiscountedPrice() > lowPrice)) {
    ...
    public static List<ProductNewBean> getProductBeanListByPrice(Users u,
        int start, int limit)
        throws JsonParseException, JsonMappingException, IOException {
        List<ProductNewBean> listOfProduct = new ArrayList<ProductNewBean>();
