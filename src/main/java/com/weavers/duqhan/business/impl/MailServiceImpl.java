...
public class MailServiceImpl implements MailService {
    ...
    @Override
    public String returnRequestToAdminDeliver(OrderDetails orderDetails) {
        String body = "";
        ProductPropertiesMap propertyMap = productPropertiesMapDao.loadById(orderDetails.getMapId());
        body = "<table><tr><td>User ID </td><td> : " + orderDetails.getUserId() + "</td></tr><tr><td>User Name</td><td> : " + usersDao.loadById(orderDetails.getUserId()).getName() + "</td></tr><tr><td>Order ID</td><td> : " + orderDetails.getOrderId() + "</td></tr><tr><td>Product</td><td> : " + productDao.loadById(propertyMap.getProductId().getId()).getName() + "</td></tr><tr><td>Product Size Color Map ID</td><td> : " + propertyMap.getId() + "</td></tr><tr><td>Quantity</td><td> : " + orderDetails.getQuentity() + "</td></tr><tr><td>Order Date</td><td> : " + DateFormater.formate(orderDetails.getOrderDate()) + "</td></tr></table>";
        String status = MailSender.sendEmail(ADMIN_MAIL, "Return Order", body, BCC);// send mail to Admin if user return a order.
        return status;
    }
    ...
    public String sendPurchaseMailToAdmin(List<OrderDetails> orderDetails) {
        if (!orderDetails.isEmpty()) {
            ...
            for (OrderDetails orderDetail : orderDetails) {
                propertyMap = productPropertiesMapDao.loadById(orderDetail.getMapId());
                String property = "";
                ...
                if (propertyValueIds.length > 0) {
                    for (String propertyValueId : propertyValueIds) {
                        try {
                            ...
                            property = property + properties.getPropertyName() + " - " + productPropertyvalues.getValueName() + "<br>";
                        } catch (Exception nfe) {
                        }
                    }
                }
                product = productDao.loadById(propertyMap.getProductId().getId());
                order = "<b>Order Id: " + orderDetail.getOrderId() + "</b><br>Product: " + product.getName() + " (" + product.getId() + ")<br>Specification: <br>" + property + "<br>Quantity: " + orderDetail.getQuentity() + "<br>Price: " + orderDetail.getPaymentAmount() + "<br><br>Delivery address :<br>";
