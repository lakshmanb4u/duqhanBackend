...
public class OrderDetailsDaoJpa extends BaseDaoJpa<OrderDetails> implements OrderDetailsDao {
    ...
    public List<Object[]> getDetailByUserId(Long userId, int start, int limit) {
        Query query = getEntityManager().createQuery("SELECT od, map FROM OrderDetails AS od, ProductPropertiesMap AS map WHERE od.userId=:userId AND od.mapId=map.id AND od.status!=:status ORDER BY od.orderDate DESC").setFirstResult(start).setMaxResults(limit);
        query.setParameter("userId", userId);
    ...
    public List<Object[]> getOrderDetailsByOrderIds(List<String> orderids) {
        ...
        String q = "SELECT od, map, u FROM OrderDetails AS od, ProductPropertiesMap AS map,Users AS u WHERE od.orderId IN(";
        int i = 0;
        String s = "";
